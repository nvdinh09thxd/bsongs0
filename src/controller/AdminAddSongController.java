...
public class AdminAddSongController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("".equals(songName)) {
			RequestDispatcher rd = request.getRequestDispatcher("/admin/song/add.jsp?msg=1");
			rd.forward(request, response);
			return;
		}
		int catId = Integer.parseInt(request.getParameter("catId"));
		if (catId == 0) {
			RequestDispatcher rd = request.getRequestDispatcher("/admin/song/add.jsp?msg=4");
			rd.forward(request, response);
			return;
		}
		String description = request.getParameter("preview");
		if ("".equals(description)) {
			RequestDispatcher rd = request.getRequestDispatcher("/admin/song/add.jsp?msg=2");
			rd.forward(request, response);
			return;
		}
		String detail = request.getParameter("detail");
		if ("".equals(detail)) {
			RequestDispatcher rd = request.getRequestDispatcher("/admin/song/add.jsp?msg=3");
			rd.forward(request, response);
			return;
		}
		// xu ly upload file
